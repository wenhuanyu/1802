	...
	export default {
		...
		onShow(option) {
			...
			uni.setNavigationBarTitle({
				title: '工序管理-'+this.data.quartel_name
			});
			this.listInterface()
		},
		methods: {
			...
			add() {
				uni.navigateTo({
					url: "/pagesA/pages/fell/process/add/add"
				})
			},
			// 编辑点击
			editClick() {
				this.show = false
				uni.navigateTo({
					url:'/pagesA/pages/fell/process/add/add?data='+encodeURIComponent(JSON.stringify(this.edit))
				})
			}
		}
	}
